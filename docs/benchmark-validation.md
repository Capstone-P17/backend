...
| 결과 | 케이스 수 | 설명 |
|---|---:|---|
| TP | 5 | 공식 ground truth가 취약이고 현재 detector가 탐지한다. |
| TN | 3 | 공식 ground truth가 비취약이고 현재 detector가 탐지하지 않는다. |
| known FN | 4 | 공식 ground truth는 취약이지만 현재 detector가 탐지하지 못한다. |
...
현재 SQL Injection, Weak Hash, Insecure Random, Command Injection 일부 패턴은 공식 샘플에서 탐지 가능하며,
분기/메서드 간 데이터 흐름이 더 필요한 Path Traversal, XSS, Hardcoded Secret 일부 샘플은 known false negative로 관리하고 있습니다.
```
...
1. Path Traversal: cookie/header 값이 중간 변수에 복사된 뒤 `FileInputStream`, `File`, `Paths.get`에 들어가는 흐름 추적
2. XSS: 분기 내부 assignment 이후 후속 sink에서 taint가 유지되도록 method-level flow 개선
3. Hardcoded Secret: declaration뿐 아니라 assignment expression의 문자열 리터럴과 sink 사용처를 함께 추적
