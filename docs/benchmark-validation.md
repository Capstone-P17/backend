...
| 구분 | 공식 소스 | 현재 결과 | 의미 |
| --- | --- | --- | --- |
| SQL Injection | OWASP BenchmarkJava | TP/TN 확인 | 취약 SQL 결합은 탐지하고, OWASP safe SQLi 샘플은 탐지하지 않는다. |
...
| Command Injection | OWASP BenchmarkJava | known FN | `List`에 명령 인자를 쌓고 `ProcessBuilder.command(argList)`로 넘기는 흐름은 아직 추적하지 못한다. |
| Path Traversal | OWASP BenchmarkJava | known FN | 쿠키 값이 여러 변수로 복사된 뒤 파일 경로에 결합되는 흐름은 아직 추적하지 못한다. |
...
현재 SQL Injection, Weak Hash, Insecure Random 일부 패턴은 공식 샘플에서 탐지 가능하며,
분기/컬렉션/메서드 간 데이터 흐름이 필요한 Command Injection, Path Traversal, XSS, Hardcoded Secret 일부 샘플은 known false negative로 관리하고 있습니다.
```

## 다음 개선 후보

1. Command Injection: `List<String>`에 쌓인 인자가 `ProcessBuilder.command(...)`로 전달되는 흐름 추적
2. Path Traversal: cookie/header 값이 중간 변수에 복사된 뒤 `FileInputStream`, `File`, `Paths.get`에 들어가는 흐름 추적
3. XSS: 분기 내부 assignment 이후 후속 sink에서 taint가 유지되도록 method-level flow 개선
4. Hardcoded Secret: declaration뿐 아니라 assignment expression의 문자열 리터럴과 sink 사용처를 함께 추적
