...
DETECTOR_METADATA: dict[str, DetectorMetadata] = {
    ...
    "DANGEROUS_FILE_UPLOAD": DetectorMetadata(
        ...
        description="업로드 파일의 형식, 크기, 개수, 실행권한, 저장 파일명 또는 저장 경로 검증이 부족하면 실행 가능한 파일이나 악성 파일이 서버에 저장될 수 있습니다.",
        recommendation="허용 확장자와 파일 시그니쳐를 함께 검증하고, 크기와 개수 제한을 적용하며, 저장 파일은 실행권한을 제거한 뒤 외부에서 식별하기 어려운 경로와 파일명으로 관리하세요.",
        safe_example=(
            ...
            "Path target = privateUploadDir.resolve(savedName);\n"
            "file.transferTo(target);\n"
            "target.toFile().setExecutable(false, false);"
        ),
        confidence="HIGH",
        confidence_reason="업로드 파일 객체가 파일 저장 API로 전달되고, 저장 전에 파일 타입, 파일 시그니쳐, 크기/개수 제한, 실행권한 제거, 서버 생성 파일명 등 필수 방어 항목 중 일부가 확인되지 않았기 때문에 HIGH로 판단했습니다.",
    ),
}
